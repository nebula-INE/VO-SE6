...
export async function renderStudioOffline(
  ...
  //
  // ★修正: 以前は休符(および解決できなかった歌詞)を配列から完全に除外
  // していた。しかしvose_core.cpp側は「配列上で隣り合うノート」を
  // 自動的に「音響的に連続している」とみなして遷移ブレンド
  // (blend_transition_spectra)を適用する設計になっている。休符を除外すると、
  // 本来は沈黙を挟んで繋がっていないはずの2つのノートが配列上だけ隣り合って
  // しまい、休符をまたいで誤った遷移ブレンドが適用され、休符明けの音の頭で
  // クリック/音色の乱れが起きていた(VCV/大型音源ほど音色差が大きく目立ちやすい)。
  //
  // vose_core.cpp の execute_render_impl には既に
  // `if (!notes[i].wav_path) { ...; prev_renderable=false; last_ev=nullptr; }`
  // という、休符で前ノート参照を正しく断ち切る仕組みが備わっているので、
  // 休符を「wav_path=nullptr(空文字列)のNoteEvent」として配列に残すだけで
  // 正しく機能する。休符の長さぶんはちゃんと無音として時間を消費する
  // (NO_VOICE状態は current_offset += pp.note_samples される)。
  const allNotesInOrder = noteWithKeys.filter(nk => nk.key === null || resolvedKeys.has(nk.key));
  if (allNotesInOrder.every(nk => nk.key === null)) {
    console.warn('[wasmEngine] 有効なノートが1つも解決できませんでした。');
    return null;
  }

  const workerNotes: WorkerNoteEntry[] = allNotesInOrder.map(({ n, key }) => {
    const noteDurationMs = (n.length / 480) * (60000 / tempo);
    const numFrames = Math.max(1, Math.round(noteDurationMs / KFRAME_PERIOD_MS));

    if (key === null) {
      // 休符: wav_path=空文字列(WASM側でnullptr相当) + ピッチカーブは
      // ダミーで良い(参照されない)が、長さ(pitch_length)は正しく渡す
      return { key: '', pitchCurveHz: new Array(numFrames).fill(0), isRest: true };
    }
    ...
    return { key, pitchCurveHz, isRest: false };
  });
